...
% Binary vs Binary-veto (C_ratio 否决门) 对比实验
% 共享种子，η=0.30
...
%% 1. 参数
fprintf('=================================================\n');
fprintf('  Binary vs Binary-veto (C_ratio 否决门)\n');
fprintf('  共享种子对比\n');
fprintf('=================================================\n\n');
...
C_veto = 30;  % 复用 dualSignalClow，噪声 95th 上界
...
out_dir = fullfile(results_dir, sprintf('binary_veto_%s', timestamp));
mkdir(out_dir);

fprintf('η = %.2f, C_veto = %d, num_runs = %d\n', eta, C_veto, num_runs);
...
%% 4. Binary-veto
fprintf('[Binary-veto] %d 次重复 (C_veto=%d)...\n', num_runs, C_veto);
R_veto = NaN(num_runs, 1);
P_veto = NaN(num_runs, 1);
...
parfor ri = 1:num_runs
    ...
    rp.useAdaptiveThreshold = true;
    rp.adaptiveThresholdConfig = adaptive_cfg;
    rp.adaptiveThresholdMode = 'binary_veto';
    rp.dualSignalClow = C_veto;
    R_veto(ri) = run_responsiveness(rp, num_angles, time_vec, seed);

    pp = pers_params;
    pp.useAdaptiveThreshold = true;
    pp.adaptiveThresholdConfig = adaptive_cfg;
    pp.adaptiveThresholdMode = 'binary_veto';
    pp.dualSignalClow = C_veto;
    P_veto(ri) = run_persistence(pp, pers_cfg, seed + 50000);
end
fprintf('  R=%.4f±%.4f, P=%.2f±%.2f\n\n', ...
    mean(R_veto,'omitnan'), std(R_veto,'omitnan')/sqrt(num_runs), ...
    mean(P_veto,'omitnan'), std(P_veto,'omitnan')/sqrt(num_runs));

%% 5. 汇总
Rb = mean(R_binary,'omitnan');  Pb = mean(P_binary,'omitnan');
Rv = mean(R_veto,'omitnan');    Pv = mean(P_veto,'omitnan');
...
fprintf('%-20s %8.4f %8.2f %+7.1f%% %+7.1f%%\n', 'Binary-veto', Rv, Pv, dR, dP);
fprintf('\n');

if dR > 0 && dP > 0
    fprintf('Binary-veto 严格优于 Binary!\n');
elseif dR >= -2 && dP > 0
    fprintf('Binary-veto: R基本持平, P提升 → 成功!\n');
elseif dR < 0 && dP > 0
    fprintf('Binary-veto: R换P (响应性下降, 持久性提升)\n');
else
    fprintf('Binary-veto: R=%+.1f%%, P=%+.1f%%\n', dR, dP);
end
...
subplot(1,2,1);
R_means = [Rb, Rv];
R_ses = [std(R_binary,'omitnan'), std(R_veto,'omitnan')] / sqrt(num_runs);
bar(R_means); hold on;
errorbar(1:2, R_means, R_ses, 'k.', 'LineWidth', 1.5);
set(gca, 'XTickLabel', {'Binary', 'Binary-veto'});
ylabel('响应性 R');
...
subplot(1,2,2);
P_means = [Pb, Pv];
P_ses = [std(P_binary,'omitnan'), std(P_veto,'omitnan')] / sqrt(num_runs);
bar(P_means); hold on;
...
sgtitle(sprintf('Binary vs Binary-veto (\\eta=%.2f, %d runs, C_{veto}=%d)', eta, num_runs, C_veto));
...
hold on;
scatter(R_binary, P_binary, 30, 'r', 'filled', 'MarkerFaceAlpha', 0.3, 'DisplayName', 'Binary (逐次)');
scatter(R_veto, P_veto, 30, 'b', 'filled', 'MarkerFaceAlpha', 0.3, 'DisplayName', 'Binary-veto (逐次)');
scatter(Rb, Pb, 200, 'r', 'p', 'filled', 'MarkerEdgeColor', [0.5 0 0], 'DisplayName', 'Binary (均值)');
scatter(Rv, Pv, 200, 'b', 'h', 'filled', 'MarkerEdgeColor', [0 0 0.5], 'DisplayName', 'Binary-veto (均值)');
xlabel('响应性 R'); ylabel('持久性 P');
title(sprintf('R-P 对比 (\\eta=%.2f, C_{veto}=%d)', eta, C_veto));
...
fprintf(fid, '========================================================================\n');
fprintf(fid, 'Binary vs Binary-veto 对比报告\n');
fprintf(fid, '生成时间: %s\n', char(datetime('now', 'Format', 'yyyy-MM-dd HH:mm:ss')));
fprintf(fid, '========================================================================\n\n');
fprintf(fid, '设置: η=%.2f, N=%d, C_veto=%d, num_runs=%d, 共享种子\n', eta, base.N, C_veto, num_runs);
...
if dR > 0 && dP > 0
    fprintf(fid, 'Binary-veto 严格优于 Binary\n');
elseif dR >= -2 && dP > 0
    fprintf(fid, 'Binary-veto: R基本持平, P提升 → 成功\n');
end
fclose(fid);

%% 8. 保存数据
saveResultBundle(out_dir, 'comparison_data', ...
    {'R_binary', 'P_binary', 'R_veto', 'P_veto', ...
     'eta', 'C_veto', 'adaptive_cfg', 'base', 'num_runs', 'base_seed'});

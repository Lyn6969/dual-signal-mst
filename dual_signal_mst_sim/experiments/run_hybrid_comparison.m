...
if dR_sig > 0 && dP_sig > 0
    fprintf('Sigmoid-gate 严格优于 Binary!\n');
elseif dR_sig < 0 && dP_sig > 0
    fprintf('Sigmoid-gate: R换P (响应性下降, 持久性提升)\n');
else
    fprintf('Sigmoid-gate: R=%+.1f%%, P=%+.1f%%\n', dR_sig, dP_sig);
end

if dR_alg > 0 && dP_alg > 0
    fprintf('Algebraic-fusion 严格优于 Binary!\n');
elseif dR_alg < 0 && dP_alg > 0
    fprintf('Algebraic-fusion: R换P (响应性下降, 持久性提升)\n');
else
    fprintf('Algebraic-fusion: R=%+.1f%%, P=%+.1f%%\n', dR_alg, dP_alg);
end
...
subplot(1,2,1);
R_means = [Rb, Rs, Ra];
R_ses = [std(R_binary,'omitnan'), std(R_sigmoid,'omitnan'), std(R_algebraic,'omitnan')] / sqrt(num_runs);
bar_h = bar(R_means); hold on;
errorbar(1:3, R_means, R_ses, 'k.', 'LineWidth', 1.5);
set(gca, 'XTickLabel', {'Binary', 'Sigmoid', 'Algebraic'});
ylabel('响应性 R');
...
subplot(1,2,2);
P_means = [Pb, Ps, Pa];
P_ses = [std(P_binary,'omitnan'), std(P_sigmoid,'omitnan'), std(P_algebraic,'omitnan')] / sqrt(num_runs);
bar_h = bar(P_means); hold on;
errorbar(1:3, P_means, P_ses, 'k.', 'LineWidth', 1.5);
set(gca, 'XTickLabel', {'Binary', 'Sigmoid', 'Algebraic'});
ylabel('持久性 P');
title(sprintf('P 对比 (\\eta=%.2f)', eta));
grid on;

sgtitle(sprintf('Binary vs Sigmoid-gate vs Algebraic-fusion (\\eta=%.2f, %d runs, 共享种子)', eta, num_runs));
...
fprintf(fid, '========================================================================\n');
fprintf(fid, 'Binary vs Sigmoid-gate vs Algebraic-fusion 对比报告\n');
fprintf(fid, '生成时间: %s\n', char(datetime('now', 'Format', 'yyyy-MM-dd HH:mm:ss')));
fprintf(fid, '========================================================================\n\n');
fprintf(fid, '设置: η=%.2f, N=%d, V_ref=%.4f, num_runs=%d, 共享种子\n', eta, base.N, V_ref, num_runs);
fprintf(fid, 'Sigmoid-gate: D_mid=0.6, κ=10, λ_base=0.3, α_min=0.3, λ_up=0.5\n');
fprintf(fid, 'Algebraic-fusion: λ_floor=λ_base×α_min=0.09, λ_up=0.5\n\n');
...
if dR_sig > 0 && dP_sig > 0
    fprintf(fid, 'Sigmoid-gate 严格优于 Binary\n');
end
if dR_alg > 0 && dP_alg > 0
    fprintf(fid, 'Algebraic-fusion 严格优于 Binary\n');
end
fclose(fid);

%% 9. 保存数据
save(fullfile(out_dir, 'comparison_data.mat'), ...
    'R_binary', 'P_binary', 'R_sigmoid', 'P_sigmoid', 'R_algebraic', 'P_algebraic', ...

%% run_response_gain_comparison.m
% Binary vs Response-Gain-Modulated Hard-Max 对比实验
% 验证“激活后响应强度调制”是否能够在尽量保住 R 的同时提升 P
...
%% 1. 参数
fprintf('=================================================\n');
fprintf('  Binary vs Response-Gain-Modulated Hard-Max\n');
fprintf('  多噪声水平快速验证\n');
fprintf('=================================================\n\n');
...
% 三种模式：Binary + 两档响应增益调制
modes = struct();
modes(1).name = 'Binary';
modes(1).useResponseGainModulation = false;
modes(1).responseGainMin = 1.0;
modes(1).responseGainBeta = 1.0;
modes(1).responseGainClow = 30;
modes(1).responseGainChigh = 150;

modes(2).name = 'GainSoft';
modes(2).useResponseGainModulation = true;
modes(2).responseGainMin = 0.6;
modes(2).responseGainBeta = 1.0;
modes(2).responseGainClow = 30;
modes(2).responseGainChigh = 150;

modes(3).name = 'GainStrong';
modes(3).useResponseGainModulation = true;
modes(3).responseGainMin = 0.4;
modes(3).responseGainBeta = 2.0;
modes(3).responseGainClow = 30;
modes(3).responseGainChigh = 150;
...
out_dir = fullfile(results_dir, sprintf('response_gain_%s', timestamp));
mkdir(out_dir);
...
for mi = 1:num_modes
    m = modes(mi);
    fprintf('  %-10s useGain=%d, g_min=%.2f, beta=%.1f, C=[%.0f, %.0f]\n', ...
        m.name, m.useResponseGainModulation, m.responseGainMin, ...
        m.responseGainBeta, m.responseGainClow, m.responseGainChigh);
end
fprintf('\n');
...
for ei = 1:num_eta
    ...
    for mi = 1:num_modes
        ...
        g_min = m.responseGainMin;
        beta = m.responseGainBeta;
        c_low = m.responseGainClow;
        c_high = m.responseGainChigh;
        ...
        parfor ri = 1:num_runs
            ...
            rp.responseGainMin = g_min;
            rp.responseGainBeta = beta;
            rp.responseGainClow = c_low;
            rp.responseGainChigh = c_high;
            ...
            pp.responseGainMin = g_min;
            pp.responseGainBeta = beta;
            pp.responseGainClow = c_low;
            pp.responseGainChigh = c_high;
...
subplot(1,2,1);
R_means = squeeze(mean(R_all, 1, 'omitnan'))';
bar(R_means);
...
sgtitle(sprintf('响应强度调制对比 (%d runs)', num_runs));
...
for mi = 1:num_modes
    m = modes(mi);
    fprintf(fid, '  %-10s useGain=%d, g_min=%.2f, beta=%.1f, C=[%.0f, %.0f]\n', ...
        m.name, m.useResponseGainModulation, m.responseGainMin, ...
        m.responseGainBeta, m.responseGainClow, m.responseGainChigh);
end
fprintf(fid, '\n');
